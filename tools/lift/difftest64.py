...
def uses_only_regs(md, insn):
    """True if every operand is a register or immediate we model."""
    if not insn.operands:
        return insn.mnemonic in ('cdqe', 'cqo', 'cdq', 'cwde', 'cbw', 'cwd',
                                 'clc', 'stc', 'cmc', 'sahf', 'lahf')
    ...
    for va in addrs:
        ...
        for ins in md.disasm(code, va):
            ...
            if m.startswith('j') or m.startswith('loop') or m[0] == 'f':
                continue
            if not uses_only_regs(md, ins):
                continue
            key = bytes(ins.bytes)
            if key not in seen:
                seen[key] = (m, ins.op_str)
        if len(seen) >= want:
            break
...
def build_harness(outdir, cases, lifter):
    ...
    table = []
    skipped = []
    for idx, (encoding, (mn, ops)) in enumerate(cases):
        name = 't_%05d' % idx
        try:
        ...
        if 'RECOMP_TODO' in body:
            ...
            continue
        lifted.append(body)
        table.append((idx, name, encoding, mn, ops))
    ...
    src.append('                 void (*fn)(CPU *); int approx; unsigned undef; } CASE;')
    src.append('')
    for idx, name, enc, mn, ops in table:
    ...
    for idx, name, enc, mn, ops in table:
        ...
        src.append('  { "%s", "%s", b_%05d, %d, %s, %d, 0x%Xu },'
                   % (mn, esc, idx, len(enc), name,
                      1 if mn in APPROX_MNEMONICS else 0,
                      UNDEFINED_FLAGS.get(mn.split()[-1], 0)))
    src.append('};')
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < ncases; i++) {
        ...
        for (int trial = 0; trial < NTRIALS; trial++) {
            ...
            /* ---- native ---- */
            memcpy(&dt_in, seed, sizeof(DTSTATE));
            ...
            CPU c;
            memset(&c, 0, sizeof(c));
            for (int k = 0; k < 16; k++) *cpu_gpr(&c, k) = seed->gpr[k];
            memcpy(c.xmm, seed->xmm, sizeof(c.xmm));
            unpack_to_cpu(&c, seed->flags);
            tc->fn(&c);
