...
## P4 — imports through the Win32 layer — **unblocked**
...
The host layer blocked this for a while and no longer does. The runtime's stop path
used `setjmp`, and in a winelib build that symbol binds to Wine's PE-side `_setjmp`
stub, whose buffer layout is not glibc's; the runtime now uses clang's built-in jump
buffer instead. After that change the winelib harness runs the same lifted functions
as the native one and agrees with it, which is what P4 was waiting for. The
investigation, including everything that was ruled out, is in
